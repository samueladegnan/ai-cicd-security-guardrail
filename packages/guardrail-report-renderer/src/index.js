 ...
 *     showChart: true,
 *     showToolbar: true,
 *     defaultSort: 'severity'   // 'severity' | 'confidence' | 'location'
 *   });
 *   renderer.render(reportJson);
 *
 * Accepted report shape (also accepts the raw guardrail JSON where each
 * result has a nested `finding` object):
 *   {
 *     summary: { total, high_priority, false_positive, unclear },
 *     results: [
 *       {
 *         filePath, line, column, ruleId, cwe, severity, message,
 *         verdict, confidence, reasoning, remediation, snippet,
 *         language, tool, complianceHits: [{ framework, ruleId, title, description }]
 *       }
 *     ]
 *   }
 */
(function (root, factory) {
  ...
  class GuardrailReportRenderer {
    ...
    _bindControls() {
      if (this.options.showToolbar) {
        ...
        filterSelect.addEventListener("change", (e) => {
          this.setVerdictFilter(e.target.value);
        });
      ...
      metricCards.forEach((card) => {
        ...
        const applyAction = () => this.setVerdictFilter(verdict, true);
    ...
    static normalizeReport(report) {
      ...
      return {
        summary: {
          ...
          high_priority: summary.high_priority ?? results.filter((r) => r.verdict === "HIGH_PRIORITY").length,
          false_positive: summary.false_positive ?? results.filter((r) => r.verdict === "FALSE_POSITIVE").length,
          unclear: summary.unclear ?? results.filter((r) => r.verdict === "UNCLEAR").length
        },
        results
      };
    }
    ...
    setVerdictFilter(verdict, toggle = false) {
      ...
      this.container.querySelectorAll(".metric-card").forEach((card) => card.classList.remove("active"));
    ...
    _applyFilters() {
      ...
      if (this.currentFilter !== "all") {
        items = items.filter((i) => i.verdict === this.currentFilter);
      }
    ...
    _renderVerdictBadge(verdict) {
      ...
      return `<span class="guardrail-badge ${cls}">${escapeHtml(label)}</span>`;
    }
    ...
    _renderComplianceHits(hits) {
      ...
      return hits
        .map(
          (h) =>
            `<span class="guardrail-badge badge-control" title="${escapeHtml(h.description || "")}">${escapeHtml(h.framework)}: ${escapeHtml(h.rule_id || h.ruleId || "")}</span>`
        )
        .join(" ");
    }
    ...
    _renderTable(items) {
      ...
      items.forEach((item, idx) => {
        ...
        row.innerHTML = `
          ...
          <td>${this._renderComplianceHits(item.complianceHits)}</td>
          <td class="verdict-cell">${this._renderVerdictBadge(item.verdict)}</td>
              ...
              <div class="detail-grid">
                ...
                <div class="detail-col detail-context">
                  ...
                  <div class="detail-block">
                    ...
                    ${item.complianceHits.length ? this._renderComplianceHits(item.complianceHits) : "<p class='muted'>None mapped</p>"}
                  </div>
                </div>
              </div>
            </div>
        ...
        detailRow.id = `${this.id}-detail-${idx}`;
        ...
        const toggleRow = () => {
          ...
          if (showing) {
            ...
            if (typeof this.options.onRowClick === "function") {
              this.options.onRowClick(item, { row, detailRow });
            }
          }
        };
